...
class AdaRRT():
        ...
        for _ in range(self.max_iter):
            try:
                ...
                if extended and self._check_for_completion(extended):
                    ...
                    extended.children.append(self.goal)
    ...
    def _get_nearest_neighbor(self, sample):
        ...
        """
        min_val, min_node = self._find_min_DFS(self.start, sample)
        return min_node

    def _find_min_DFS(self, node, sample):
        cur_min = np.sum((node.state - sample) ** 2)
        min_node = node

        for child in node.children:
            new_num, new_node = self._find_min_DFS(child, sample)
            if (new_num < cur_min):
                cur_min = new_num
                min_node = new_node

        return cur_min, min_node
